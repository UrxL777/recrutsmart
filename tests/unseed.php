<?php
// tests/unseed.php
// Supprime tous les candidats de test et leurs CV (et toutes les données liées)
...
foreach ($candidatsTest as $c) {
    ...
    // Supprimer les enregistrements dans les tables liées (au cas où, bien que CASCADE devrait fonctionner)
    $pdo->prepare('DELETE FROM cv_analyses WHERE candidat_id = ?')->execute([$c['id']]);
    $pdo->prepare('DELETE FROM candidatures WHERE candidat_id = ?')->execute([$c['id']]);
    $pdo->prepare('DELETE FROM messages WHERE candidat_id = ?')->execute([$c['id']]);
    $pdo->prepare('DELETE FROM conversations_agent WHERE user_id = ? AND user_role = "candidat"')->execute([$c['id']]);
    $pdo->prepare('DELETE FROM resultats_matching WHERE candidat_id = ?')->execute([$c['id']]);
