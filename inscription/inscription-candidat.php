...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf($_POST['csrf_token'] ?? '')) {
        ...
        $old = compact('nom','prenom','email','tel','ville','question');
        ...
        if (!$ville)                                   $erreurs[] = 'La ville est requise.';
        ...
        if (!$erreurs) {
            $st = $pdo->prepare('SELECT id FROM candidats WHERE email=?');
            $st->execute([$email]);
            if ($st->fetch()) {
        ...
        if (!$erreurs) {
            $st = $pdo->prepare('INSERT INTO candidats
                (nom,prenom,email,telephone,ville,mot_de_passe,question_securite,reponse_securite)
                VALUES (?,?,?,?,?,?,?,?)');
            $st->execute([
            ...
            createSession((int)$pdo->lastInsertId(), 'candidat', $nom, $prenom);
            redirectToDashboard();
        }
    }
}
...
<title>Inscription Candidat — RecrutSmart</title>
<style>
...
<div class="card">
  ...
  <h1>Créer votre espace candidat</h1>
  <p class="subtitle">Remplissez le formulaire — vous serez connecté automatiquement</p>
  ...
  <form method="POST" action="inscription-candidat.php" novalidate>
    ...
    <div class="grid-2">
      <div class="form-group">
        ...
        <input type="text" id="nom" name="nom"
          value="<?= clean($old['nom'] ?? '') ?>" placeholder="Koné" required>
      </div>
      <div class="form-group">
        <label for="prenom">Prénom *</label>
        <input type="text" id="prenom" name="prenom"
          value="<?= clean($old['prenom'] ?? '') ?>" placeholder="Aya" required>
      </div>
    </div>
    ...
    <div class="form-group">
      ...
      <input type="email" id="email" name="email"
        ...
        placeholder="aya.kone@email.com" required autocomplete="email">
    </div>
